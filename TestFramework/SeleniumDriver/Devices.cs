...
namespace TeamControlium.TestFramework
{
    public partial class SeleniumDriver
    {
        ...
        {
            string device = Utilities.TestData[ConfigDevice[0], ConfigDevice[1]];

            if (string.IsNullOrEmpty(device)) throw new SeleniumExceptions.RunOptionOrCategoryDoesNotExist(ConfigDevice[0], ConfigDevice[1], "Unable to set Selenium host Device.");
            switch (device.ToLower().Replace(" ", string.Empty).Replace("(", string.Empty).Replace(")", string.Empty))
            {
                ...
                default:
                    throw new SeleniumExceptions.UnsupportedDevice(device);
            }
        }
    }
