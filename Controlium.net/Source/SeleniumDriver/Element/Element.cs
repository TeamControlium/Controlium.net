...
namespace TeamControlium.Controlium
{
    public partial class Element
    {
        ...
        public bool WaitForElementHeightStable(TimeSpan? Timeout = null)
        {
            ...
            Log.LogWriteLine(Log.LogLevels.FrameworkInformation, "Element height stable after {0}ms ({1} itterations)", timeWaited.Elapsed.TotalSeconds.ToString(), Itterations.ToString());
        ...
        public bool WaitForElementPositionStable(TimeSpan? Timeout = null)
        {
            ...
            Log.LogWriteLine(Log.LogLevels.FrameworkInformation, "Element position stable after {0}ms ({1} itterations)", timeWaited.Elapsed.TotalSeconds.ToString(), Itterations.ToString());
            return didStabilzeBeforeTimeout;
        }
            ...
            try
            {
                while (++retryIndex <= maxretries || maxretries == 0)
                {
                    try
                    {
                        ...
                        EnterText(Text);
                        if (retryIndex > 1)
                            Log.LogWriteLine(Log.LogLevels.FrameworkInformation, "{0} attempts with InvalidElementStateException.  Last attempt good.)", retryIndex);
                        return;
                    }
        ...
        public void EnterText(string Text,int maxretries= 3, TimeSpan? retryInterval = null)
        {
            ...
            try
            {
                while (++retryIndex <= maxretries || maxretries == 0)
                {
                    ...
                    {
                        Log.LogWriteLine(Log.LogLevels.FrameworkInformation, "Attempt {0} failed (InvalidElementStateException)", retryIndex);
                        Thread.Sleep(interval);
                        lastException = ex;
                    }
                }
