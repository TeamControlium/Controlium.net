...
namespace TeamControlium.Controlium
{
    public partial class SeleniumDriver
    {
        ...
        {
            string device = Repository.GetItemGlobalOrDefault(ConfigDevice[0], ConfigDevice[1],default(string));

            if (device==default(string)) throw new RunOptionOrCategoryDoesNotExist(ConfigDevice[0], ConfigDevice[1], "Unable to set Selenium host Device.");
