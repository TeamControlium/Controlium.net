...
namespace TeamControlium.Controlium
{
    public partial class SeleniumDriver
    {
        ...
        public void SetBrowserSize(string browserSize)
        {
            try
            {
                if (browserSize.ToLower().Trim().Replace(" ", "").Contains("full"))
                {
                    Log.LogWriteLine(Log.LogLevels.TestInformation, $"TestBrowserSize is [{browserSize}] - Setting browser fullscreen");
        ...
        public bool IsElementVisible(Element Element, bool CheckIfElementIsInViewport)
        {
            if ((Element != null) && Element.WebElement != null)
            {
                if (CheckIfElementIsInViewport)
                {
                    ...
                    Log.LogWriteLine(Log.LogLevels.FrameworkInformation, "Element in Viewport = {0} ({1})", sResult, MyDisplayed ? "Yes" : "No");
            ...
            {
                Log.LogWriteLine(Log.LogLevels.FrameworkInformation, "Element displayed = [Web or element is NULL]");
                return false;
            }
        }
        ...
        public bool Displayed(IWebElement webElement)
        {
            ...
            Log.LogWriteLine(Log.LogLevels.FrameworkInformation, "Element displayed = [{0}]", result ? "TRUE" : "FALSE");
            return result;
        }
        ...
        public bool IsElementEnabled(Element Element)
        {
            ...
            {
                Log.LogWriteLine(Log.LogLevels.FrameworkInformation, "Element enabled = [Web or element is NULL]");
                return false;
            }
        }
        ...
        public bool Enabled(IWebElement webElement)
        {
            ...
            Log.LogWriteLine(Log.LogLevels.FrameworkInformation, "Element enabled = [{0}]", result ? "TRUE" : "FALSE");
            return result;
        }
        ...
        {
            Log.LogWriteLine(Log.LogLevels.FrameworkInformation, "Clearing element");
            WebElement.Clear();
        }
        ...
        public bool IsSelected(IWebElement WebElement)
        {
            ...
            Log.LogWriteLine(Log.LogLevels.FrameworkInformation, "Is Element Selected = {0}", result ? "TRUE" : "FALSE");
            return result;
        }
        ...
        {
            Log.LogWriteLine(Log.LogLevels.FrameworkInformation, "Clicking element (Selenium click action)");
            try
            {
                WebElement.Click();
            }
                        ...
                        if (loc.Length > start && loc.Contains(')'))
                        {
                            ...
                            if (loc.Contains(','))
                            {
                                ...
                                if (TryResult && df != null)
                                {
                                    ...
                                    throw new ElementCannotBeClicked(string.Format("Element could not be clicked as another element would get the click.  The offending element is (Element and all children shown):\r\n{0}", OffendingElement), ex);
                                }
                            }
                        }
                    }
        ...
        public void SetText(IWebElement WebElement, string Text)
        {
            ...
            //IWebElement aa = ElementFindTimeout.Until((b) => { if (IsElementVisible(WebElement) && IsElementEnabled(WebElement)) return WebElement; else { Log.WriteLn(this, "SetText", "Polling until Text can be entered"); return null; } });
            Log.LogWriteLine(Log.LogLevels.FrameworkInformation, "Entering textusing Selenium SendKeys: [{0}].", Text);
        ...
        public string GetText(IWebElement WebElement, bool IncludeDescendentsText, bool ScrollIntoViewFirst, bool UseInnerTextAttribute)
        {
            ...
            if (IncludeDescendentsText)
            {
                if (UseInnerTextAttribute)
                {
                    ...
                    Log.LogWriteLine(Log.LogLevels.FrameworkInformation, "Get element text using element innerText attribute: [{0}]", text);
                }
                else
                {
                    text = WebElement.Text;
                    Log.LogWriteLine(Log.LogLevels.FrameworkInformation, "Get element text using Selenium Text property: [{0}]", text);
                }
            }
            else
            {
        ...
        {
            Log.LogWriteLine(Log.LogLevels.FrameworkInformation, "Scrolling element in to view (JavaScript inject - [Element].scrollIntoView()");
        ...
        public bool WaitForElement(Element ElementToWaitFor, Visibility RequiredVisibility, TimeSpan? Timeout = null, TimeSpan? PollInterval = null)
        {
            ...
            try
            {
                DidReachRequiredVisibilityBeforeTimeout = actualTimeout.Until<bool>((d) =>
                {
                    ...
                    try
                    {
                        ...
                        Log.LogWriteLine(Log.LogLevels.FrameworkDebug, $"[{timeWaited.ElapsedMilliseconds}mS] Element displayed (exists and isDisplayed true): {isDisplayed}");
                        if (isDisplayed)
                        {
                    ...
                    {
                        Log.LogWriteLine(Log.LogLevels.TestInformation, $"Unexpected exception waiting for element to be visible or not visible: {ex}");
                        throw;
                    }
            ...
            {
                Log.LogWriteLine(Log.LogLevels.FrameworkInformation, "WebDriverTimeoutException after {0}ms ({1} itterations)", timeWaited.Elapsed.TotalSeconds.ToString(), Itterations.ToString());
                DidReachRequiredVisibilityBeforeTimeout = false;
            }
            catch (Exception ex)
            {
                Log.LogWriteLine(Log.LogLevels.FrameworkInformation, "Exception after {0}ms ({1} itterations)", timeWaited.Elapsed.TotalSeconds.ToString(), Itterations.ToString());
                throw new WaitTimeout(ElementToWaitFor.ParentOfThisElement,
                    ElementToWaitFor,
        ...
        public bool WaitForElement(Element ParentElement, ObjectMappingDetails Mapping, Visibility RequiredVisibility, bool CheckIsDisplayedStatus, TimeSpan? Timeout = null, TimeSpan? PollInterval = null)
        {
            ...
            Log.LogWriteLine(Log.LogLevels.FrameworkInformation, "Wait {0}ms for element {1} to become {2} (Check displayed status = {3}). Poll interval = {4}ms", actualTimeout.Timeout.TotalMilliseconds, Mapping.FriendlyName, RequiredVisibility.ToString(), CheckIsDisplayedStatus, actualTimeout.PollingInterval.TotalMilliseconds);
            ...
            try
            {
                DidReachRequiredVisibilityBeforeTimeout = actualTimeout.Until((d) =>
                {
                    ...
                    try
                    {
                        ...
                        Log.LogWriteLine(Log.LogLevels.FrameworkInformation, "[{0}mS]: Element {1} ", timeWaited.ElapsedMilliseconds, (WebElementFound == null) ? "not found" : $"found (Displayed - {WebElementFound.IsDisplayed})");
                        Itterations++;
                    ...
                    catch (Exception ex)
                    {
                        if (RequiredVisibility == Visibility.Hidden)
                        {
                            Log.LogWriteLine(Log.LogLevels.FrameworkInformation, "Exception thrown but element not visible wanted so success. Exception: {0}", (WebElementFound == null) ? "Element not found" : "Element found but not displayed", ex.Message);
                            HaveWeSuccess = true;
                        }
                    }
                    return HaveWeSuccess;
            ...
            {
                Log.LogWriteLine(Log.LogLevels.FrameworkInformation, "WebDriver Timeout Exception after {0}ms ({1} itterations)", timeWaited.Elapsed.TotalMilliseconds.ToString(), Itterations.ToString());
                DidReachRequiredVisibilityBeforeTimeout = false;
            }
            catch (Exception ex)
            {
