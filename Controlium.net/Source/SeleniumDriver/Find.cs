...
namespace TeamControlium.Controlium
{
    public partial class SeleniumDriver
    {
        ...
        public bool TryFindElement(Element ParentElement, ObjectMappingDetails Mapping, bool AllowMultipleMatches, TimeSpan? TimeoutOverride, TimeSpan? PollOverride, bool WaitUntilStable, out Element element)
        {
            ...
            Log.LogWriteLine(Log.LogLevels.FrameworkDebug, "Timeout - {0} Seconds", webDriverWait.Timeout.TotalSeconds);
            ...
            {
                Log.LogWriteLine(Log.LogLevels.FrameworkInformation, "Found {0} matching elements in {1} mS.{2}", clauseResults.Count.ToString(), timer.ElapsedMilliseconds.ToString(), AllowMultipleMatches ? " Allow multiple matches so using first match." : string.Empty);
        ...
        internal List<Element> FindElements(Element ParentElement, ObjectMappingDetails Mapping)
        {
            ...
            else
            {
                ...
                Log.LogWrite(Log.LogLevels.FrameworkDebug, "Find Elements: Name - [{0}], Selenium logic - [{1}]", Mapping.FriendlyName, SeleniumFindBy.ToString());
                try
                {
                ...
                Log.LogWriteLine(Log.LogLevels.FrameworkDebug, " (Found [{0}] elements)", FoundElements.Count.ToString());
        ...
        public List<Element> FindElements(ObjectMappingDetails Mapping)
        {
            ...
            //
            By SeleniumFindBy = Mapping.SeleniumBy;
            Log.LogWrite(Log.LogLevels.FrameworkInformation, "Find Elements: {0}", SeleniumFindBy.ToString());
            try
            {
            ...
            Log.LogWriteLine(Log.LogLevels.FrameworkInformation, " (Found [{0}] elements)", FoundElements.Count.ToString());
