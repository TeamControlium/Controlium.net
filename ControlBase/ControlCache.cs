...
namespace TeamControlium.Framework
{
    ...
    public static class ControlCache
    {
        ...
        public static void Add(ControlBase NewControl)
        {
            ...
            {
                throw new Exception("FATAL - Control [" + NewControl.FindLogic?.FriendlyName?? NewControl.FindLogic?.FindLogic??"Unknown!!  No find login or friendly name" + "] already exists - why did we not reuse it (on invalidate if stale?)?!");
            }
        }
        ...
        public static ControlBase Get(ControlBase FoundControlToGet)
        {
            ...
            throw new Exception("CoreLib_Control:ControlCollection - cannot locate control: " + FoundControlToGet.FindLogic?.FriendlyName ?? FoundControlToGet.FindLogic?.FindLogic ?? "Unknown!!  No find login or friendly name");
        }
        ...
        private static bool ControlMatch(ControlBase Control1, ControlBase Control2)
        {
            ...
            if (Compare(Control1?.FindLogic?.FindLogic?.ToString()??null, Control2?.FindLogic?.FindLogic?.ToString()??null))
            {
                _control1 = Control1;
                _control2 = Control2;
                ...
                while (_control1.ParentControl!=null)
                {
                    ...
                    {
                        if (!Compare(_control1?.FindLogic?.FindLogic?.ToString() ?? null, _control2?.FindLogic?.FindLogic?.ToString() ?? null))
                        {
