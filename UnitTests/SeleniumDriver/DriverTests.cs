    ...
    public class DriverTests
    {
        ...
        public void Selenium_can_be_launched_with_mandatory_settings_set()
        {
            foreach (dynamic browser in browsers)
            {
                ...
                // Assert 
                Assert.IsTrue(Process.GetProcessesByName(browser.ServerProcessName).Count() > 0, $"There must be more than one process named ({browser.ServerProcessName})");
                Assert.IsTrue(Process.GetProcessesByName(browser.ProcessName).Count() > 0, $"There must be more than one process named ({browser.ProcessName})");
