...
{
    /// <summary>Provides API for test scripts and LGIAsuper libraries.  Encapsulates extensions to Selenium WebDriver and a layer of abstraction that gives a unified and LGIAsuper
    ...
    public partial class SeleniumDriver
    {
        ...
        private readonly string[] SeleniumDebugFile = { "Selenium", "DebugLogFile" };                // Path and file for Selenium Log file.  Default is the console window
        ...
        /// <summary>
        /// Displays a prompt for user to enter Credentials.
        /// </summary>
        /// <param name="Title">Tital of dialog</param>
        /// <returns>Credentials entered by user</returns>
        static public Credentials GetCredentials(string Title = null)
        {
            ...
            using (UserCredentialsDialog dialog = new UserCredentialsDialog())
            {
                ...
                if (dialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    return new Credentials(dialog.User, dialog.PasswordToString());
                }
                else
        ...
        public string TakeScreenshot(string Suffix = null)
        {
            ...
            try
            {
                    ...
                    if ((Utilities.TestData.TryGetItem("Iest","ID", out TestID)))
                    {
                        ...
                        if (TotalPathLength > 248)
                        {
                            ...
                            if ((TotalPathLength - TestID.Length - 2) > 248)
                            {
                                ...
                                Logger.WriteLine(Logger.LogLevels.TestInformation, "Total path length ({0}) greater than 248 and TestID length is {1} so cannot fix path length by truncating TestID",TotalPathLength, TestID.Length);
                ...
                {
                    throw new NotImplementedException("webDriver does not implement ITakesScreenshot!  Is it LgiaRemoteWebDriver?");
                }
            }
            catch (Exception ex)
            {
                Logger.WriteLine(Logger.LogLevels.TestInformation, "Exception saving screenshot [{0}]: {1}", Filename,ex);
            }
            return string.Empty;
        }
        ...
        public void CloseDriver()
        {
            ...
            try
            {
                TakeScreenshotOption = Utilities.TestData["Debug", "TakeScreenshot"];
            }
            catch (Exception ex)
            {
        ...
        private DesiredCapabilities GetCapabilities(string HostName)
        {
            ...
            foreach (KeyValuePair<string, dynamic> capability in Utilities.TestData.GetCategoryItems(HostName))
            {
        ...
        private void SetupRemoteRun()
        {
            ...
            string seleniumHost = Utilities.TestData[ConfigHost[0], ConfigHost[1]];
            DesiredCapabilities requiredCapabilities = GetCapabilities(seleniumHost);

            // Now get the Selenium Host details
            Uri uri;
            string seleniumHostURI = Utilities.TestData[ConfigHostURI[0], ConfigHostURI[1]];
            if (!Uri.TryCreate(seleniumHostURI, UriKind.Absolute, out uri)) throw new SeleniumExceptions.InvalidHostURI(seleniumHostURI);

            // And connection timeout
            int connectionTimeout = Utilities.TestData[ConfigConnectionTimeout[0], ConfigConnectionTimeout[1]];
            ...
            {
                throw new SeleniumExceptions.SeleniumWebDriverInitError(uri.AbsoluteUri, requiredCapabilities.ToString(), ex);
            }
        ...
        private string CheckAndPreparSeleniumLogFile(string SeleniumDebugFile)
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    sw.WriteLine("Selenium Debug File");
                    sw.Close();
                    return PathAndFile;
                }
                catch (Exception ex)
        ...
        private void SetupLocalRun()
        {
            ...
            string seleniumFolder = Utilities.TestData[SeleniumServerFolder[0], SeleniumServerFolder[1]] ?? ".";
            bool seleniumDebugMode = Utilities.IsValueTrue(Utilities.TestData[SeleniumDebugMode[0], SeleniumDebugMode[1]]);
            string seleniumDebugFile = Utilities.TestData[SeleniumDebugFile[0], SeleniumDebugFile[1]] ?? string.Empty;

            // Check the folder exists and chuck a wobbly if it doesnt...
            if (!Directory.Exists(seleniumFolder)) throw new SeleniumExceptions.SeleniumFolderError(seleniumFolder);
            ...
            try
            {
                ...
                if (IsEdge)
                {
                    ...
                    EO.PageLoadStrategy = EdgePageLoadStrategy.Eager;
                ...
                if (webDriver == null) throw new SeleniumExceptions.SeleniumWebDriverInitError(seleniumFolder, IsInternetExplorer ? "Internet Explorer" : IsChrome ? "Chrome" : IsEdge ? "Edge" : "Undefined!!");

            }
            catch (Exception ex)
            {
                throw new SeleniumExceptions.SeleniumWebDriverInitError(seleniumFolder, IsInternetExplorer ? "Internet Explorer" : IsChrome ? "Chrome" : IsEdge ? "Edge" : "Undefined!!", ex);
            }
        }
        private void SetTimeouts()
        {
            ...
            elementFindTimings = new WebDriverWait(webDriver, TimeSpan.FromMilliseconds((Utilities.TestData.TryGetItem(ConfigTimeout[0], ConfigTimeout[1], out dummy)) ? dummy : defaultTimeout));
            elementFindTimings.PollingInterval = TimeSpan.FromMilliseconds((Utilities.TestData.TryGetItem(ConfigPollingInterval[0], ConfigPollingInterval[1], out dummy)) ? dummy : defaultPollInterval);

            if (Utilities.TestData.TryGetItem(ConfigPageLoadTimeout[0], ConfigPageLoadTimeout[1], out dummy))
            {
                PageLoadTimeout = TimeSpan.FromMilliseconds(dummy);
            }
            else
            {
                throw new Exception($"Error setting Page load timeout!  Has Run Category Option [{ConfigPageLoadTimeout[0]},{ConfigPageLoadTimeout[1]}] been set?");
            }
        }
