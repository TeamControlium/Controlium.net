...
namespace TeamControlium.Framework
{
    public partial class SeleniumDriver
    {
        ...
        private static bool TestBrowserHasBeenSet { get; set; } = false;
        /// <summary>
        ...
        public static void SetTestBrowser()
        {
            ...
            string browser;
            if (!Utilities.TestData.TryGetItem<string>(ConfigBrowser[0], ConfigBrowser[1], out browser))
            {
                throw new SeleniumExceptions.InvalidSettings(string.Format("Error getting setting [{0}],[{1}].", ConfigBrowser[0], ConfigBrowser[1]), Utilities.TestData.TryException);
            }
