    ...
    public partial class SeleniumDriver
    {
        ...
        public bool TryFindElement(Element ParentElement, ObjectMappingDetails Mapping, bool AllowMultipleMatches, TimeSpan? TimeoutOverride, TimeSpan? PollOverride, bool WaitUntilStable, out Element element)
        {
            ...
            if (clauseResults.Count > 1 && !AllowMultipleMatches)
            {
                ...
                TryException = new SeleniumExceptions.FindLogicReturnedMultipleElements(((ParentElement == null) ? "DOM Top Level" : (string.IsNullOrEmpty(ParentElement.MappingDetails.FriendlyName) ? ("Unknown Parent (" + ParentElement.MappingDetails.FindLogic + ")") : ParentElement.MappingDetails.FriendlyName)), Mapping, clauseResults.Count);
                element = null;
                return false;
            }
            ...
            else
            {
                ...
                TryException = new SeleniumExceptions.FindLogicReturnedNoElements(ParentElement?.MappingDetails?.FriendlyName??"No parent (searched from top level of DOM)", Mapping, webDriverWait.Timeout.TotalSeconds.ToString(), webDriverWait.PollingInterval.TotalMilliseconds.ToString());
                element = null;
                return false;
            }
        }
        ...
        internal List<Element> FindElements(Element ParentElement, ObjectMappingDetails Mapping)
        {
            ...
            else
            {
                ...
                {
                    throw new SeleniumExceptions.SeleniumFindElementError(Mapping, ParentElement, ex);
                }
        ...
        public List<Element> FindElements(ObjectMappingDetails Mapping)
        {
            ...
            {
                throw new SeleniumExceptions.SeleniumFindElementError(Mapping, this, ex);
            }
